...
public class WallBottomMesh(GraphicsDevice device, RawSector sector, RawWall wall, RawWall nextWall)
    : IDisposable
{
    ...
    public void LoadContent()
    {
        try
        {
            ...
            var picnum = wall.Picnum;

            if (wall.IsBottomTextureSwapped)
                picnum = State.LoadedRawMap.Walls[wall.NextWall].Picnum;

            var tile = State.LoadedGroupArt.Tiles[picnum];
    ...
    private void LoadBottomWall()
    {
        ...
        if (nextSector == null)
            return; // Skip if wall has no next sector

        var top = sector.FloorZ;
        var bottom = sector.FloorZ;

        if (sector.FloorZ >= nextSector.FloorZ)
            top = nextSector.FloorZ;
        //if (!wall.IsWallAlignedBottom)
        //top = sector.FloorZ;
    ...
    private void LoadWallBuffers(RawSector sector, int bottom, int top)
    {
        ...
        var wallPoints = new List<Vector3>
        {
            ...
            MapHelper.ConvertDuke3DToMono(new Vector3(nextWall.RawX, nextWall.RawY, bottom)),
            MapHelper.ConvertDuke3DToMono(new Vector3(nextWall.RawX, nextWall.RawY, top)),
        ...
        var wallWidth = Vector2.Distance(
            ...
            new Vector2(nextWall.RawX, nextWall.RawY)
        );
